...
- **Tarjeta por notificación, compacta**: cada `NotificationListItem` es un contenedor propio, **solo** el borde izquierdo coloreado por `type` (nunca un borde completo). El fondo es plano (no noisy — el noisy vive únicamente en el `Popover` contenedor, vía `getNoisyBackgroundSx`, para que la tarjeta se note como una capa encima): no leída = un tono más claro que el contenedor (`theme.custom.white` en claro / `theme.custom.background` en oscuro); leída = **el mismo color que el contenedor** (`theme.custom.lightBackground`/`darkBackground`, la misma fórmula que usa `getNoisyBackgroundSx` por default), para que se "camufle" en vez de seguir compitiendo visualmente. El único cambio entre leída/no leída es de color — nunca de `fontWeight`.
- **80% mensaje / 20% tiempo + acciones**: dentro de la tarjeta, el título ocupa `flex: '1 1 80%'` con `noWrap` + ellipsis, y una columna angosta a la derecha (`flex: '0 0 20%'`) apila el tiempo relativo arriba y las dos acciones (ojo, flecha) abajo — así el mensaje nunca se corta antes de tiempo por culpa de las acciones.

...
const ProductDialogValidationSchema = Yup.object().shape({
    ...
    Presentation: Yup.object().shape({
      ...
      stock: Yup.number().min(0, "El stock debe ser mayor a 0").required("Campo requerido"),
      updated_at: Yup.string().required("Campo requerido"),
    }).required("Campo requerido"),
    requiredStock: Yup.number().moreThan(0, "El stock requerido debe ser mayor a 0").required("Campo requerido"),

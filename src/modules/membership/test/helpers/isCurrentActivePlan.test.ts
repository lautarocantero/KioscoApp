...
describe("isCurrentActivePlan", () => {
    it("devuelve false si no hay status todavía (cargando)", () => {
        expect(isCurrentActivePlan(null, KioscoPlanEnum.Stocko)).toBe(false);
    });

    it("devuelve true cuando el plan coincide y está activo", () => {
        const status = buildStatus({ plan: KioscoPlanEnum.SuperStocko, plan_status: KioscoPlanStatusEnum.Active });
        expect(isCurrentActivePlan(status, KioscoPlanEnum.SuperStocko)).toBe(true);
    });

    it("devuelve false cuando el plan coincide pero el pago está pendiente", () => {
        const status = buildStatus({ plan: KioscoPlanEnum.SuperStocko, plan_status: KioscoPlanStatusEnum.PendingPayment });
        expect(isCurrentActivePlan(status, KioscoPlanEnum.SuperStocko)).toBe(false);
    });

    it("devuelve false cuando el plan no coincide", () => {
        const status = buildStatus({ plan: KioscoPlanEnum.Stocko, plan_status: KioscoPlanStatusEnum.Active });
        expect(isCurrentActivePlan(status, KioscoPlanEnum.MaxiStocko)).toBe(false);
    });
});

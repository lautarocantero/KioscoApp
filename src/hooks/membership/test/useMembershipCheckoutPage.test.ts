...
describe("useMembershipCheckoutPage", () => {
    ...
    it("pay() dispara startCheckout con el id del plan resuelto", () => {
        ...
        mockedUseMembershipCheckoutPlan.mockReturnValue({ plan: KioscoPlanEnum.SuperStocko, planDefinition: buildPlan(), loading: false, error: null });
        mockedUseMembershipCheckout.mockReturnValue({ isSubmitting: false, error: null, startCheckout });

        const { result } = renderHook(() => useMembershipCheckoutPage("super_stocko"));
        result.current.pay();

        expect(startCheckout).toHaveBeenCalledWith(KioscoPlanEnum.SuperStocko);
    });
    ...
    it("expone isSubmitting/checkoutError del hook de checkout", () => {
        mockedUseMembershipCheckoutPlan.mockReturnValue({ plan: KioscoPlanEnum.Stocko, planDefinition: buildPlan(), loading: false, error: null });
        mockedUseMembershipCheckout.mockReturnValue({ isSubmitting: true, error: "No se pudo iniciar el pago", startCheckout: vi.fn() });

        const { result } = renderHook(() => useMembershipCheckoutPage("stocko"));
